            ...
            for region in region_names:
                try:
                    ...
                    try:
                        ...
                        nw_firewall_dict = nw_client.list_firewalls()
                        for nw_fw in nw_firewall_dict['FirewallPolicies']:
                            firewall_arn = nw_fw['Arn']
                            nw_fw_response = nw_client.describe_firewall_policy(FirewallArn=firewall_arn)
                            nw_fw_response['account_id'] = session_name
                            temp_list.append(nw_fw_response)
                    except Exception as e:
                        print(e)
                except Exception as e:
                    print(e)
        with open('nw_fw_policy.json', 'w') as f:
...
if __name__ == '__main__':
    ...
    parser = Parser()
    df_dict = parser.process(response, 'nw_firewall_policy')
