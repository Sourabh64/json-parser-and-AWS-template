                ...
                try:
                    ...
                    try:
                        ...
                        for resp in resp_list:
                            lc = resp['LaunchConfigurationName']
                            resp_lc_list = client.describe_launch_configurations(LaunchConfigurationNames=[lc])['LaunchConfigurations']
                            for resp_lc in resp_lc_list:
                                imageid = resp_lc['ImageId']
                                ec2_client = boto3.client('ec2', **cred)
                                resp_dict = ec2_client.describe_images(ImageIds=[imageid])
                                resp_dict['asg_name'] = resp['AutoScalingGroupName']
                                resp_dict['launch_conf'] = lc
...
if __name__ == '__main__':
    ...
    parser = Parser()
    df_dict = parser.process(response, 'asg_image')
    db = DB()
